...
task.spawn(function()
    while task.wait(0.1) do
        if AutoPickupEnabled then
            -- Loop qua các item rơi trên floor (Thay Workspace.DroppedItems bằng Folder chứa item thực tế trong game)
            for _, item in pairs(workspace:GetChildren()) do
                if item:FindFirstChild("TouchInterest") or item:IsA("BasePart") then
                    -- Kiểm tra điều kiện Rarity & Stats ở đây nếu item có BillboardGui/Attributes
                    local hrp = game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                    if hrp and (item.Position - hrp.Position).Magnitude < 30 then
